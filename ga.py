...
class GA:
    ...
    def mutate(self, pop):
        ...
        # atualizando o sigma
        if self.porcentagem_sucesso > 0.3:
            self.sigma = self.sigma / 0.9
        elif self.porcentagem_sucesso < 0.3:
            self.sigma = self.sigma * 0.9
    ...
    def run(self):
        ...
        for i in range(self.max_iter):
            ...
            print('iter: {}, actual fitness: {:.6f}, best_fitness: {:.6f}'.format(i, float(fitness.max()), float(max([max(x) for x in self.history]))), end=", ")
            print("p_success: {:.6f}, sigma: {}".format(self.porcentagem_sucesso, self.sigma))
